...
class TenderRepository implements TenderInterface
{
    ...
    {
        // Display only pending tenders for admin
        if (Auth::user()->role->id == 3) {
            $user = Auth::user();
            $pendingTenders = $user->tender()->where('status', '1')->orderBy('id', 'DESC')->get();
            return $pendingTenders;
        }

        // Display only pending tenders
        return $pendingTenders = Tender::where('status', '1')->orderBy('id', 'DESC')->get();
    }
}
