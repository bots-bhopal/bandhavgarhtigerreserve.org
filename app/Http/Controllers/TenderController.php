...
class TenderController extends Controller
{
    ...
    {
        $tenders = $this->tender->pendingTenders();

        // Show Tender
        if (Auth::user()->role->id == '3' && View::exists('admin.tender.pending')) {
            return view('admin.tender.pending', compact('tenders'));
        } else if (Auth::user()->role->id == '2' && View::exists('superadmin.tender.pending')) {
            return view('superadmin.tender.pending', compact('tenders'));
        } else if (Auth::user()->role->id == '1' && View::exists('serveradmin.tender.pending')) {
            return view('serveradmin.tender.pending', compact('tenders'));
        }
    }
}
