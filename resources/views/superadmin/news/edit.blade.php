            ...
            <div class="row">
                <div class="col-lg-8 col-md-12">
                    <div class="card card-primary card-outline">
                        ...
                        <form action="{{ route('superadmin.newses.update', $newses->id) }}" method="POST"
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control @error('title') is-invalid @enderror"
                                            id="title" name="title" value="{{ $newses->title }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control mb-2 @error('date') is-invalid @enderror"
                                        id="date" name="date" value="{{ $newses->date }}">
                                    @error('date')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="ckeditor form-control mb-2 @error('description') is-invalid @enderror"
                                        rows="5" id="description"
                                        name="description">{{ $newses->description }}</textarea>
                                    @error('description')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="card-footer">
                                <button type="submit" class="btn btn-primary"><i class="nav-icon fa fa-pencil-alt"
                                        style="margin-right: 5px;"></i>{{ __('panel.news-btn-update') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="card card-outline card-primary">
                        ...
                        <div class="card-body">
                            ...
                            @if (!empty($newses['image']) && file_exists($path))
                                <img src="{{ asset('public/storage/news_image/' . $newses->image) }}" width="100%"
                                    height="100%" class="img-responsive rounded" alt="image">
                            @else
                                <img src="{{ asset('public/assets/images/' . $newses->image) }}" width="100%"
                                    height="100%" class="img-responsive rounded" alt="image">
                            @endif
                        </div>
                        ...
                        @if (!empty($newses['image']) && file_exists($path))
                            <div class="card-footer d-flex">
                                <button type="button" class="btn btn-danger btn-sm"
                                    onclick="deleteImage({{ $newses->id }})">Delete Image<i
                                        class="fas fa-trash-alt pl-1"></i></button>

                                <form id="delete-image-form-{{ $newses->id }}"
                                    action="{{ route('superadmin.newses.destroy_image', $newses->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                                ...
                                                @if (!empty($image['image']) && file_exists($path))
                                                    <div class="card-footer d-flex">
                                                        ...
                                                        <form id="delete-images-form-{{ $image->id }}"
                                                            action="{{ route('superadmin.newses.destroy_images', $image->id) }}"
